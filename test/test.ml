...
module ParseLineStartTests = struct
  ...
  let testable_markdownLineStarter = testable (Reader.MarkdownLineStarter.pp) (Reader.MarkdownLineStarter.equal)
  ...
  let test3 () = check (Alcotest.option testable_markdownLineStarter) "Header 1" (Some (Reader.MarkdownLineStarter.Header 1)) start2
  let test4 () = check string "Header 1" "This is a header 1 sentence" (rest2 |> List.to_seq |> String.of_seq)
  let test5 () = check (Alcotest.option testable_markdownLineStarter) "Header 2" (Some (Reader.MarkdownLineStarter.Header 2)) start3
  let test6 () = check string "Header 2" "This is a header 2 sentence" (rest3 |> List.to_seq |> String.of_seq)
  let test7 () = check (Alcotest.option testable_markdownLineStarter) "Bullet Point" (Some Reader.MarkdownLineStarter.UnorderedListItem) start4
